# CodeBuddy MCP setup guide

This branch adds CodeBuddy support to CodeGraph. Use this guide on another
machine, such as a company computer, to install the branch, configure CodeBuddy,
generate the knowledge graph, and use it from the CodeBuddy agent.

## What this branch provides

- Adds `codebuddy` as a CodeGraph installer target.
- Writes CodeBuddy MCP config for:
  - CodeBuddy IDE global config: `~/.codebuddy/mcp.json`
  - Project-local config: `<project>/.mcp.json`
- Passes `--path <project>` to the MCP server so VS Code, Android Studio, and
  CodeBuddy IDE launch CodeGraph against the correct workspace.
- Adds the MCP tool `codegraph_index`, so the CodeBuddy agent can initialize,
  update, or rebuild the knowledge graph from natural language.
- Keeps the normal graph reading tools available, such as `codegraph_status`,
  `codegraph_explore`, `codegraph_search`, `codegraph_node`,
  `codegraph_callers`, and `codegraph_impact`.

## Install this branch on a company computer
...
Open a terminal at the project you want CodeBuddy to understand:

```bash
cd /absolute/path/to/your/company/project
```
  ...
  "mcpServers": {
    "codegraph": {
      ...
      "args": [
        ...
        "--mcp",
        "--path",
        "/absolute/path/to/your/company/project"
      ],
...
You can check status from the terminal:
...
1. Open the company project in CodeBuddy.
...
Check the project-local `.mcp.json` has an absolute `--path` pointing at the
project opened in CodeBuddy.
...
The local Node runtime does not have SQLite FTS5 support. Use the CodeGraph
self-contained runtime when available, or run CodeGraph with a Node build that
includes `node:sqlite` with FTS5.

For this development branch, the simplest fallback is to run from a machine
whose Node supports CodeGraph's SQLite requirements, then install with:
